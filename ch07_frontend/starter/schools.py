...
class SchoolManager:
    ...
    def get_all_schools(self):
        SCHOOLS_SQL = 'SELECT school_id, fullname, city, state, country FROM schools'
        results = []
        with closing(sqlite3.connect(self.db_filename)) as conn:
            cursor = conn.cursor()
            cursor.execute(SCHOOLS_SQL)

            for record in cursor:
                results.append(School(*record))

            results.sort(key=lambda s: vars(s).get('fullname'))
...
if __name__ == '__main__':
    ...
    print(len(SchoolManager('./course_data.db').get_all_schools()))
